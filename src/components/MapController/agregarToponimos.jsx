...
const ToponimosLayer = ({
  ...
  useEffect(() => {
    ...
    toponimos.forEach((toponimo, index) => {
            ...
            <img class="imagenNumeros" src="${mapName == "TNATransformadoras" || mapName == "MOrienteCali" ? fondoMarker  :image}"
              style="width: ${
                mapName == "bredunco" && [2, 3, 4, 1].includes(toponimo.numero)
                  ? "25px"
                  : [2, 3, 4, 6, 7, 8, 9, 10, 11, 14, 15, 26, 28, 32].includes(
                      toponimo.numero
                    )
                  ? "30px"
                  : "25px"
      ...
      popupNode.addEventListener("mouseenter", () => {
        const popupContainer = popupNode.closest('.maplibregl-popup');
        if (popupContainer) popupContainer.style.zIndex = "10000";
        if (tooltip) {
          positionTooltip();
          tooltip.style.opacity = "1";
        }
      ...
      popupNode.addEventListener("mouseleave", () => {
        const popupContainer = popupNode.closest('.maplibregl-popup');
        if (popupContainer) popupContainer.style.zIndex = "";
      ...
      popupNode.addEventListener("click", () => {
        if (mapName == "bredunco") {
          setModalIndex(index + 23);
          setIsModalOpen(true);
        } else if (mapName == "ASuarez") {
          setModalIndex(index + 31);
          setIsModalOpen(true);
        } else if (mapName == "AVillaRica") {
          setModalIndex(index + 38);
          setIsModalOpen(true);
        } else if (mapName == "VDOrienteCali") {
          setModalIndex(index + 43);
          setIsModalOpen(true);
        }else if (toponimos[0].capa === "Nodo Suárez") {
          onMapChange(index + 1);
        } else if (toponimos[0].capa === "Nodo Finca El Buhido") {
          onMapChange(index + 1);
        }else if (toponimos[0].capa === "Nodo ASOYOGE") {
          onMapChange(index + 1);
        } else if (mapName == "MOrienteCali") {
          setModalIndex(index + 52);
          setIsModalOpen(true);

  console.log("modalData[index + 52]")
      ...
      if (
        mapName == "ASuarez" ||
        mapName == "AVillaRica" ||
        mapName == "VDOrienteCali" ||
        mapName == "introduccionCap4"

      ) {

      const circulo = popupNode.querySelector(".circuloPopup");
        const root = createRoot(circulo ? circulo: popupNode); // Usamos `createRoot` para React 18
  ...
  {(isModalOpen &&
        modalIndex !== null && mapName === "MOrienteCali") && (
